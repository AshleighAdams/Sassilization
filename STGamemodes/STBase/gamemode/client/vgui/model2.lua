...
/*---------------------------------------------------------
   Name: OnSelect
---------------------------------------------------------*/
function PANEL:SetHatModel( strModelName, posHat )
...
function PANEL:Paint()
	...
	if IsValid(self.HatEntity) && self.HatEntity.HatPos then
		...
		if attachment then 	
			...
			self.HatEntity:SetPos(attachment.Pos + (attachment.Ang:Up() * HatInfo[4]) + (attachment.Ang:Right() * Right) + (attachment.Ang:Forward() * (HatInfo[6] or 0)) + Add)
...
derma.DefineControl( "DSassilizationModelPanel", "A panel containing a model", PANEL, "DModelPanel" )